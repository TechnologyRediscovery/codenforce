...
public class CodeIntegrator extends BackingBeanUtils implements Serializable {
    ...
    /**
     * Utility method for use by methods in this Integrator which allows easy
     * filling of code-related objects with fully-baked CodeSource objects Note
     * that client methods must manage cursor positins on the ResultSet passed
     * in.
     *
     *
     * @param rs
     *
     * @param source Clients are responsible for instantiating an emtpy
     * CodeSource and passing a reference into this method
     *
    ...
    /**
     * Utility method for converting a ArrayList of CodeSource objects into a
     * map with the name of the CodeSource as the key and the CodeSource object
     * as the value. Designed for selectItem components that the view likes to
     * eat for breakfast.
     *
        ...
        try {
            ...
            if (con != null) {
                try {
                    ...
                    System.out.println("getCodeSetById | " + e.toString());
                }
            }
        } // close finally
        return cs;
    ...
    /**
     * Creates what we call an EnforcableCodeElement, which means we find an
     * existing code element and add muni-specific enforcement data to that
     * element and store it in the DB
     *
     * This operation adds an entry to table codesetelement and uses the ID of
     * the codeSet and CodeElement to make the many-to-many links in the
     * database.
     *
    ...
    /**
     * Adds a code set listing which is then a container for enforcable code set
     * elements
     *
     * @param codeSetToInsert
     * @return the code set list (each code set does not have the list of
     * enfcodeelements)
     * @throws IntegrationException
     */
    ...
    /**
     * Creates and populates single EnforcableCodeElement object based on giving
     * ID number.
     *
    ...
    /**
     * Returns a ArrayList of fully-baked EnforcableCodeElement objects based on
     * a search with setID. Handy for then populating data tables of codes, such
     * as in creating a codeviolation. This involves a rather complicated object
     * composition process that draws on several other methods in this class for
        ...
        try {
            ...
            if (con != null) {
                try {
                    ...
                    System.out.println("getEnforcableCodeElementList | " + e.toString());
                }
            }
        } // close finally
    ...
    /**
     * Internal Utility method for loading up a CodeElement object given a
     * result set. Used by all sorts of CodeElement related methods that are
     * getting data for many code elements back in a single result set. Note
     * that the client method is responsible for manging ResultSet cursor
     * position
     *
     * @param rs With cursor positioned at the row to extract and populate from
     * @param e an empty CodeElement. Client class is responsible for
     * instantiation
    ...
    /**
     * Core method which grabs data related to a single CodeElement and
     * populates a CodeElement object which is returned to the client.
     *
     * @param elementID
    ...
    /**
     * Key method for returning a fully-assembled link of code elements by
     * source ID. Each code element returned by a single call to this method
     * contains its own instance of a CodeSource object whose field values are
     * identical.
     *
     * NOTE these are CodeElement that can become EnforcableCodeElement when
     * they are added to a codset (which is, in turn, associated with a single
     * muni)
     *
     * @param sourceID the CodeSource id used in the WHERE clause of the
     * embedded SQL statment
    ...
    public void insertCodeElement(CodeElement element) throws IntegrationException {
        String query = "INSERT INTO public.codeelement(\n"
                ...
                + "            ordhumanfriendlytext, isactive, resourceurl, datecreated)\n"
                + "    VALUES (DEFAULT, ?, ?, ?, \n"
                + "            ?, ?, ?, ?, ?, \n"
                + "            ?, ?, ?, now());";
