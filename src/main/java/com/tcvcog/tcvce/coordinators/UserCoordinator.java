...
public class UserCoordinator extends BackingBeanUtils implements Serializable {
     ...
     */
    public UserAuthorized requestUserMuniAuthorization(String loginName, Municipality targetMuni)  
            throws      IntegrationException, 
                        AuthorizationException{

        UserIntegrator ui = getUserIntegrator();    
        User usr = null;
        UserAuthorized au = null;
        Map<Municipality, UserAuthorizationPeriod> muniAuthMap = null;
    ...
    private User configureUserMuniAccess(UserAuthorized userWAccess, Municipality m) throws AuthorizationException{
        ...
        if( m.getMuniCode() == userWAccess.getGoverningAuthPeriod().getMuni_municode()
                &&
            userWAccess.getGoverningAuthPeriod().getAccessgranteddatestart().isBefore(LocalDateTime.now())
                &&
            userWAccess.getGoverningAuthPeriod().getAccessgranteddatestop().isAfter(LocalDateTime.now())
                &&
            userWAccess.getGoverningAuthPeriod().getRecorddeactivatedTS() == null){
                ...
                // current UserAuthorizationPeriod
                if(userWAccess.getGoverningAuthPeriod().getSupportstartdate() != null
                        && userWAccess.getGoverningAuthPeriod().getSupportstopdate() != null
                        && userWAccess.getGoverningAuthPeriod().getSupportstartdate().isBefore(LocalDateTime.now())
                        && userWAccess.getGoverningAuthPeriod().getSupportstopdate().isAfter(LocalDateTime.now())){
                    userWAccess.setRoleType(RoleType.Developer);
                    userWAccess.setKeyCard(getAccessKeyCard(userWAccess.getRoleType()));
                    return userWAccess;
                } else if(userWAccess.getGoverningAuthPeriod().getSysadminstartdate() != null
                        && userWAccess.getGoverningAuthPeriod().getSysadminstopdate() != null
                        && userWAccess.getGoverningAuthPeriod().getSysadminstartdate().isBefore(LocalDateTime.now())
                        && userWAccess.getGoverningAuthPeriod().getSysadminstopdate().isAfter(LocalDateTime.now())){
                    userWAccess.setRoleType(RoleType.SysAdmin);
                    userWAccess.setKeyCard(getAccessKeyCard(userWAccess.getRoleType()));
                    return userWAccess;
                } else if(userWAccess.getGoverningAuthPeriod().getCodeofficerstartdate() != null
                        && userWAccess.getGoverningAuthPeriod().getCodeofficerstopdate() != null
                        && userWAccess.getGoverningAuthPeriod().getCodeofficerstartdate().isBefore(LocalDateTime.now())
                        &&userWAccess.getGoverningAuthPeriod().getCodeofficerstopdate().isAfter(LocalDateTime.now())){
                    userWAccess.setRoleType(RoleType.EnforcementOfficial);
                    userWAccess.setKeyCard(getAccessKeyCard(userWAccess.getRoleType()));
                    return userWAccess;
                } else if(userWAccess.getGoverningAuthPeriod().getStaffstartdate() != null
                        && userWAccess.getGoverningAuthPeriod().getStaffstopdate() != null
                        && userWAccess.getGoverningAuthPeriod().getStaffstartdate().isBefore(LocalDateTime.now())
                        && userWAccess.getGoverningAuthPeriod().getStaffstopdate().isAfter(LocalDateTime.now())){
...
//        u.setPassword(tempPassword);
    ...
    /**
     * Updates all fields on the User EXCEPT password which is updated separately
     * @param u
     * @throws IntegrationException 
     */
    ...
    /**
     * Integrator method is responsible for hashing the pass. The UI will validate length.
     * @param u
     * @param pw
     * @throws IntegrationException
     * @throws AuthorizationException 
