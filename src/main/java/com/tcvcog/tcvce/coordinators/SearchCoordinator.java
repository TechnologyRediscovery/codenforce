...
//        if(query.getUser().getRoleType().getRank() > query.getQueryName().getUserRankMinimum() ){
    ...
    public QueryCEAR assembleQueryCEAR(QueryCEAREnum qName, UserAuthorized u, Municipality m, SearchParamsCEActionRequests params) throws IntegrationException{
      ...
      */
     public SearchParamsCEActionRequests generateParams_CEAR_RequestorCurrentU(Municipality m, UserAuthorized u) throws IntegrationException{
...
//        }
     ...
     * @return 
     */
    public List<Query> getEventQueryList(UserAuthorized u, Municipality m){
...
//        queryList.add(eq);
    ...
    public SearchParamsEventCECase getSearchParamsEventsRequiringAction(UserAuthorized u, Municipality muni){
    ...
    public SearchParamsEventCECase getSearchParamsOfficerActivity(UserAuthorized u, Municipality m){
        // event types are always bundled in an EventCategory
        // so in this case of this query, we don't care about the Category title,
        // only the type
...
//        if(query.getUser().getRoleType().getRank() > query.getQueryName().getUserRankMinimum() ){
    ...
    public QueryPerson assembleQueryPerson(QueryPersonEnum qName, UserAuthorized u, Municipality m, SearchParamsPerson params){
         ...
         if(qName.getRequiredRoleMin() != null){
            query.setUserRankAccessMinimum(qName.getRequiredRoleMin());
         } else {
            query.setUserRankAccessMinimum(RoleType.MuniReader);
         }
    ...
    private SearchParamsPerson generateParams_persons_users(){
        ...
        ptl.add(PersonType.User);
        params.setPersonTypes(ptl);
